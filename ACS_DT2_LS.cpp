...
// 2-opt local search improvement function with time limit
vector<int> two_opt_improve_structured(vector<int> route, chrono::high_resolution_clock::time_point start_time, chrono::seconds timeout) {
    if (route.size() < 4) return route;
    
    vector<int> best_route = route;
    double best_fitness = fitness_eval(route);
    bool improved = true;
    ...
    while (improved) {
        ...
        // Try swapping pairs of consecutive nodes (truck-drone pairs)
        for (int i = 0; i < route.size() - 3; i += 2) {
            for (int j = i + 2; j < route.size() - 1; j += 2) {
                ...
                // Swap two consecutive pairs
                swap(new_route[i], new_route[j]);
                swap(new_route[i + 1], new_route[j + 1]);
                
                double new_fitness = fitness_eval(new_route);
        ...
        // Try reversing segments while maintaining truck-drone pairing
        for (int i = 0; i < route.size() - 3; i += 2) {
            for (int j = i + 2; j < route.size() - 1; j += 2) {
                // Check time limit
                auto current_time = chrono::high_resolution_clock::now();
                ...
                // Reverse segment of pairs
                for (int k = i; k <= j; k += 2) {
                    int reverse_pos = j - (k - i);
                    if (reverse_pos > k) {
                        swap(new_route[k], new_route[reverse_pos]);
                        swap(new_route[k + 1], new_route[reverse_pos + 1]);
                    }
                }
...
void route_construction(vector<int>v)
{
	...
	int iteration = 0;
	while (true) {
        // Check if 5 seconds have elapsed for ACS
        auto current_time = chrono::high_resolution_clock::now();
        ...
        // Dynamic parameter updates based on progress
        double progress = min(1.0, chrono::duration_cast<chrono::milliseconds>(current_time - acs_start_time).count() / 5000.0);
        r0 = progress;
        p1 = 1.0 - progress;
        p2 = progress;
		...
		for(int ant = 1; ant <= num_ants; ant+=2)
		{
			...
			// Apply 2-opt local search to improve the route with remaining time
			auto remaining_time = acs_timeout - chrono::duration_cast<chrono::seconds>(current_time - acs_start_time);
			if (remaining_time.count() > 0) {
                r = two_opt_improve_structured(r, acs_start_time, acs_timeout);
            }
	...
	// Final 2-opt improvement on the global best route with any remaining time
	auto final_time = chrono::high_resolution_clock::now();
    auto remaining_time = acs_timeout - chrono::duration_cast<chrono::seconds>(final_time - acs_start_time);
    if (remaining_time.count() > 0) {
        global_best_route = two_opt_improve_structured(global_best_route, acs_start_time, acs_timeout);
        global_best_fitness = fitness_eval(global_best_route);
    }
